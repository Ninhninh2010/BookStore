									...
									<table id="example1" class="table table-bordered table-striped">
										...
										<tbody>
											<c:forEach var="i" begin="0" end="${size-1 }">
												<tr>
													<td><a
														href="Ad_AccountControl?maKH=${list[i].getMaKH()}">${list[i].getMaKH()}</a>
													</td>
														...
														<c:if test="${tt.maTrangThai == list[i].getMaTrangThai()}">
															...
															<c:if test="${list[i].getMaTrangThai() == 2}">
																<td><span class="badge badge-success"
																	style="background: black">${tt.tenTrangThai}</span></td>
															</c:if>
															<c:if test="${list[i].getMaTrangThai() == 3}">
																<td><span class="badge badge-success"
																	style="background: blue">${tt.tenTrangThai}</span></td>
															</c:if>
															<c:if test="${list[i].getMaTrangThai() == 4}">
																<td><span class="badge badge-success"
																	style="background: red">${tt.tenTrangThai}</span></td>
															</c:if>
															<c:if test="${list[i].getMaTrangThai() == 5}">
																<td><span class="badge badge-success"
																	style="background: orange">${tt.tenTrangThai}</span></td>
															</c:if>
														</c:if>
